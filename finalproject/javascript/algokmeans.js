...
function produceData(dataset_index) {
    ...
    if (dataset_index == 1) {
        ...
        for (i = 0; i < datasize; i++) {
            group1_x.push(Math.floor(Math.random() * (15 - 1) + 1))
            group1_y.push(Math.floor(Math.random() * (15 - 1) + 1))
        }
...
function generateData(dataset_index) {
    ...
    // generate dataset
    generated_data = produceData(dataset_index)
...
function generateCentroid() {
    ...
    if (converged) {
        $("#centroid_error").text("Converged! View Results in Step 4.")
...
function displayConvergenceInformation() {
    ...
    $("#centroid_1_md").text("Centroid 1 Coordinates: (" + centroids_x[0].toFixed(2) + ", " + centroids_y[0].toFixed(2) + ")");
    $("#centroid_2_md").text("Centroid 2 Coordinates: (" + centroids_x[1].toFixed(2) + ", " + centroids_y[1].toFixed(2) + ")");
...
function nextIteration() {
    ...
    if (!centroidGenerated) {

        // console.log("centroids not generated")

        $("#next_iter_error").text("Centroids not generated. Go to step 2 before proceeding.")
        return
    }
